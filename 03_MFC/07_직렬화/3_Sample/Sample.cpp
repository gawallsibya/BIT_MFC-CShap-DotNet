...
BOOL CSampleApp::InitInstance()
{
	...
	// 1. App는 SingleDocTemplate을 만들어서 포인터를 보관한다.
	CSingleDocTemplate* pDocTemplate;
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,  // 리소스 ID(메뉴, icon, string t, accel)
		RUNTIME_CLASS(CSampleDoc), // 동적 생성을 지원하기 위해.
...
/*
	...
	// 3. Frame을 만들고, View를 생성한다.
	CFrameWnd* p = pDocTemplate->CreateNewFrame(pDoc, 0);
	// 4. Frame을 초기화 한다. view와 doc이 초기화 된다.
	pDocTemplate->InitialUpdateFrame( p, pDoc);
	m_pMainWnd = p;
*/
	...
	if (!ProcessShellCommand(cmdInfo)) // frame, doc 생성..
		return FALSE;
